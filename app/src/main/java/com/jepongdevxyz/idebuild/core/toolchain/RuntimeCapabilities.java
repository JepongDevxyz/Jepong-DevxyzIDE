...
public final class RuntimeCapabilities {
    ...
    private RuntimeCapabilities(File javaHome, File gradleExecutable, File androidSdk,
                                File aapt2, boolean ready, List<String> missingRequirements) {
        this.javaHome = javaHome;
        this.gradleExecutable = gradleExecutable;
        this.androidSdk = androidSdk;
        this.aapt2 = aapt2;
        this.ready = ready;
        this.missingRequirements = Collections.unmodifiableList(new ArrayList<String>(missingRequirements));
    }
    ...
    public static RuntimeCapabilities inspect(File appFilesDir, ProjectRequirements requirements) {
        ...
        return new RuntimeCapabilities(javaHome, gradle, sdk, aapt2, missing.isEmpty(), missing);
    }
    ...
    public String getMissingRequirement() {
        if (missingRequirements.isEmpty()) return null;
        StringBuilder out = new StringBuilder();
        for (int i = 0; i < missingRequirements.size(); i++) {
            if (i > 0) out.append("; ");
            out.append(missingRequirements.get(i));
        }
        return out.toString();
    }
}
